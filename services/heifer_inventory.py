...
def build_heifer_inventory_csv(report: dict[str, Any], selected_farms: list[str]) -> str:
    return build_age_inventory_csv(report, selected_farms)


def build_heifer_inventory_pdf(report: dict[str, Any], selected_farms: list[str]) -> bytes:
    return build_age_inventory_pdf(
        report,
        selected_farms,
        title="Heifer Inventory",
        empty_message="No heifers match the selected filters.",
    )
