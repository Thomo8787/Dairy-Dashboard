...
class RotaryEntryEmailService:
    ...
    def __init__(self, download_dir: str | Path | None = None):
        ...
        self.download_dir = Path(download_dir) if download_dir else Path("data") / "rotary_entry"
    ...
    def _search_messages(self, top: int = 40) -> list[dict]:
        ...
        url = (
            ...
            f'?$search="subject:Rotary Entry ID"'
            f"&$top={top}"
    ...
    def fetch_rotary_entry_csvs(self, top: int = 40) -> list[dict]:
        downloads: list[dict] = []
